...
class gcal_combiner:
   ...
   def addCalendarData(calendar_user, visibility, cal_name, font_color):
      new_cal_data = []
   ...
   def fetchCalendarEvents(days_in_future):
      for cal_data in self.calendar_data:
         ...
         end_date = time.strftime("%Y-%m-%d", time.gmtime(time.time() + (days_in_future * 86400)))
